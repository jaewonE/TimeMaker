...
- Configurable active-timer click extension: clicking either the minute or second value adds 1–60 minutes without adding that time to Analytics.
...
- While a timer is running or paused, click either its minute or second value to add the configured number of minutes. This is enabled by default with a one-minute increment. Click-added time extends the countdown but is excluded from the recorded Analytics duration.
...
- Click extension and scroll adjustment are independent. Active scroll changes continue to change the recorded Analytics duration.
...
| --- | --- | --- |
| Extend active timers by clicking | On | On / Off; applies when either timer number is clicked |
| Click extension | 1 min | 1–60 minutes per click; excluded from Analytics |
